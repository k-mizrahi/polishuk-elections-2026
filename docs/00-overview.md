...
| Polls-chart span (2026-08-19) | The chart shows the **last 4 weekly averages** — a fixed number of points, not a date window. A date window was tried first (`min(4 weeks ago, launch day)`) and rejected the same session: it empties the chart entirely whenever ingest has been down longer than the window, which is exactly the state the game was in when it was written. Counting points instead degrades gracefully — a scraping gap widens the calendar span the 4 points cover rather than blanking the chart. Fewer than 4 are drawn only when the data does not reach back that far |
