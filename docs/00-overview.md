...
| Polls-page chart shows weekly averages (2026-08-19) | The chart on `polls.html` plots the **Sunday-to-Saturday average** — the quantity the game actually scores ([02](02-scoring-spec.md) §2) — not one point per poll. Hovering or tapping a point names the polls behind it. Buckets key off `fieldwork_end`, not `game_week_id`, which is null for polls fielded outside the game's schedule. The page fetches every approved poll rather than a page of them, because a `limit` cuts mid-week and a partial week's average is wrong; the table view still lists the most recent 60 |
| Polls-chart window (2026-08-19) | The chart starts at **min(4 weeks before today, launch day)**, snapped back to that date's Sunday so a partial week is never plotted. Before launch and through its first month this keeps pre-launch polling history on screen; from ~4 weeks after launch onward the start pins to launch day and the window simply grows with the game. Launch day lives in `app_settings.launch_date` (not a frontend constant — the date has already moved twice and changing it must not need a redeploy); if it is unset the chart degrades to the 4-week window |
