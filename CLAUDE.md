...
**Poll ingest is down, deliberately parked.** The scraper has failed every run since 2026-08-07 on `Unmapped column: 'Unity'` (tripwire issue #4 open). Per the 2026-08-19 decision, no per-party registry work happens until the final lists land **Thursday 2026-09-10** — the registry is rebuilt against them in one pass, and the first green scrape after that backfills everything, because a run re-parses the whole Wikipedia table. The **`scrape` cron is paused** as of 2026-08-20 (62 consecutive failures were sending ~4 emails a day against an accepted condition) — the `schedule:` block in `.github/workflows/scrape.yml` is commented out and **must be uncommented on 09-10**; `workflow_dispatch` still works, and the fail-loud tripwire is untouched. So expect: four standing watchdog alerts until 09-10, and a review queue that keeps growing (8 pending, oldest scraped 2026-07-11). **The 09-10→09-11 window is still tight, but shorter** — registry rebuild, backfill scrape, review-queue clearing, mock-data teardown and the gate flip; the `game_weeks` regeneration was pulled out of it and is done. Everything else is green — `weekly-close` and `watchdog` run clean, pipeline suite 56/56, frontend build clean.

✅ **The live `game_weeks` are on R8** (done 2026-08-20): all 38 real weeks remapped in place to Sunday→Saturday with the lock at the week's own Sunday 00:00 IL, IDs preserved. Verified 0 non-Sunday starts, 0 weeks where `lock_at` ≠ the week-start instant, exactly one open week. Migration `0007` (field-format checks) is applied too. Nothing is owed to the live DB before launch except the mock-data teardown above.
